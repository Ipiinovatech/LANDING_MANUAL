...
export function VideoPlayer({ videoUrl, title }: VideoPlayerProps) {
  ...
  useEffect(() => {
    if (!isIOS && Hls.isSupported() && currentVideoUrl.endsWith(".m3u8")) {
      const hls = new Hls({
        enableWorker: true,
        lowLatencyMode: true,
        backBufferLength: 90,
  ...
  return (
    <div
      ...
      className={`relative w-full max-w-5xl mx-auto aspect-video bg-black rounded-xl overflow-hidden shadow-lg ${
          ...
          <ReactPlayer
            ...
            config={{
              file: {
                ...
                forceVideo: true,
                forceHLS: currentVideoUrl.endsWith(".m3u8"),
                hlsOptions: {
                  enableWorker: true,
                  autoStartLoad: true,
                  startPosition: -1,
